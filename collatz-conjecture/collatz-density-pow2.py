...
# Quick sanity check
test_cases = [(3, (2, 5)), (5, (1, 3)), (7, (5, 11)), (27, (41, 70))]
...
def is_power_of_2(n):
    """Returns True iff n is an exact power of 2."""
...
def verify_statements(start, end):
    ...
    # Streak tracking: consecutive run of equal T values
    # streak_counts maps streak_length -> {"freq", "first_start", "last_start"}
    streak_counts  = {}
    streak_start   = start
    streak_len     = 1

    def record_streak(length, s_start):
        if length not in streak_counts:
            streak_counts[length] = {"freq": 0,
                                     "first_start": s_start,
                                     "last_start":  s_start}
        streak_counts[length]["freq"]       += 1
        streak_counts[length]["last_start"]  = s_start
    ...
    for n in range(start, end):
        ...
        # Update streak using T_curr vs the running streak T
        # T of n+1 is T_curr; streak_T is implicitly T of streak_start
        # We detect a break when same_T is False
        if same_T:
            streak_len += 1
        else:
    ...
    return counts, counterexamples, streak_counts


def print_report(counts, counterexamples, streak_counts, start, end):
    print("\n" + "="*65)
    ...
    # Streak length frequency table
    print(f"\n  {'Streak':>8}  {'Frequency':>12}  {'First start':>14}  {'Last start':>14}")
    print(f"  {'-'*8}  {'-'*12}  {'-'*14}  {'-'*14}")
    for length in sorted(streak_counts):
        rec = streak_counts[length]
        print(f"  {length:>8}  {rec['freq']:>12,}  "
              f"{rec['first_start']:>14,}  {rec['last_start']:>14,}")
    print("="*65)
...
t0 = time.time()
counts, counterexamples, streak_counts = verify_statements(START, END)
elapsed = time.time() - t0

print_report(counts, counterexamples, streak_counts, START, END)
